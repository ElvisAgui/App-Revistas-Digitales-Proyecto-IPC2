...
CREATE TABLE revista(titulo VARCHAR(35) NOT NULL, editor VARCHAR(35) NOT NULL,  categoria VARCHAR(30) NOT NULL, precio INT NOT NULL, costo_Global INT NOT NULL, suscripcion BOOLEAN NOT NULL, reaccionar BOOLEAN NOT NULL, comentar BOOLEAN NOT NULL,  CONSTRAINT PK_titulo PRIMARY KEY (titulo), CONSTRAINT FK_editor FOREIGN KEY (editor) REFERENCES usuario(nombre), CONSTRAINT FK_categori_revista FOREIGN KEY (categoria) REFERENCES categoria(nombre_Categoria));
...
CREATE TABLE etiqueta_anuncio(id INT auto_increment, anuncio VARCHAR(60) NOT NULL, etiqueta VARCHAR(25) NOT NULL, CONSTRAINT PK_etiqueta_anuncio PRIMARY KEY (id),CONSTRAINT FK_etiquet_anuncio FOREIGN KEY (etiqueta) REFERENCES etiqueta(nombre_Etiqueta), CONSTRAINT FK_anuncio FOREIGN KEY (anuncio) REFERENCES anuncio(id));
